...
<body>
	...
	<section>
		<div class="form1">
			<h2>Choose The Customer</h2>
		<form method="post">
  <div class="input-box">
            <label>Customers</label>
            <select name="customer">
              <?php
              $se = mysqli_query($conn,"SELECT *FROM customer");
...
<div class="table">
			<table border="1px">
				...
				$generation = mysqli_query($conn,"SELECT *FROM orders WHERE CUSTOMER_ID='$customer'");
				if ($generation == true) {
				...
				?><tr>
						...
						<th colspan="2"><?php
						$generation = mysqli_query($conn,"SELECT *FROM orders WHERE CUSTOMER_ID='$customer'");
						$row=mysqli_fetch_array($generation);
						$customer=$row['CUSTOMER_ID']; 
						$cus=mysqli_query($conn,"SELECT sum(PURCH_AMT)FROM orders WHERE CUSTOMER_ID='$customer'");
						$ro= mysqli_fetch_array($cus);
 						echo $ro['0']; ?> RWF</th>
					</tr><?php }} else{ ?>
						 <tr>
